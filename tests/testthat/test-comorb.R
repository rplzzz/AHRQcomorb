...
empty <- rep(0, nrow(comorb_description))
names(empty) <- comorb_description$comorbidity

## result vectors for the test patients
r1 <- empty ; r1[c('alcohol','depress','wghtloss')] <- 1
r2 <- empty ; r2[c('alcohol','depress')] <- 1
r3 <- empty ; r3[c('chf','htn_cx','renlfl_sev','alcohol','liver_mld')] <- 1
r4 <- empty ; r4[c('liver_sev','diab_cx','htn_cx','renlfl_sev','cancer_mets')] <- 1
r5 <- empty ; r5['cancer_mets'] <- 1
r6 <- r5
r7 <- empty ; r7['cancer_solid'] <- 1
r8 <- empty ; r8[c('liver_mld', 'diab_uncx', 'htn_uncx', 'renlfl_mod', 'cancer_nsitu')] <- 1
